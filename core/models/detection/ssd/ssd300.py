...
import numpy as np
import torch
from .base.vgg16 import vgg16_config
from .base.resnet50 import resnet50_config
...
class SSD300(nn.Module):
    ...
    # config.update(vgg16_config)
    config.update(resnet50_config)
    ...
    def __init__(self, n_classes):
        ...
        for i, ar in enumerate(self.config["aspect_ratios"]):
            n = len(ar) * 2 + 2
            self.locs.append(nn.Conv2d(n_channels[i], n * 4, kernel_size=3, padding=1))
            self.confs.append(nn.Conv2d(n_channels[i], n * self.n_classes, kernel_size=3, padding=1))
